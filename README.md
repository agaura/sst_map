...
On touchscreens, tap or drag the flat map to move the pinned analysis location continuously. Scroll the page outside the map; pinch zoom remains available on the flat map. Drag the timeline to select a frame and pause playback. The pinned location's coordinates and current temperature appear beneath the timeline. Sliders have expanded touch targets around their visible thumbs. Reduced-motion preferences disable initial color-cloud auto-rotation; its checkbox can override that choice.
